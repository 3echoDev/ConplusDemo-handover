...
import { Upload, Search, Grid3X3, List, FileText, CheckCircle2, Clock, AlertCircle, Eye, Check, X, Edit } from "lucide-react";
    ...
    <div className="space-y-6 animate-slide-in">
      ...
      <div className={cn("grid gap-6", selectedInvoice ? "grid-cols-1 xl:grid-cols-2" : "grid-cols-1")}>
                    ...
                    <tr key={inv.id} className={cn("border-b border-border last:border-0 hover:bg-secondary/40 transition-colors cursor-pointer", selectedInvoice === inv.id && "bg-primary/5")} onClick={() => setSelectedInvoice(inv.id)}>
                      ...
                      <td className="p-4">
                        <div className="flex items-center justify-center gap-1">
                          {inv.status === "received" || inv.status === "pending-review" ? (
                            <>
                ...
                <div key={inv.id} className={cn("rounded-xl border border-border bg-card p-5 shadow-sm hover:shadow-md transition-all", selectedInvoice === inv.id && "ring-2 ring-primary")}>
                  ...
                  {inv.status === "received" || inv.status === "pending-review" ? (
                    <div className="flex gap-2 pt-3 border-t border-border">
                      <button className="flex-1 inline-flex items-center justify-center gap-1 px-3 py-2 rounded-md bg-success/10 text-success hover:bg-success hover:text-success-foreground transition-colors text-xs font-medium">
                        <Check className="h-3 w-3" /> Approve
                      </button>
                      <button className="flex-1 inline-flex items-center justify-center gap-1 px-3 py-2 rounded-md bg-destructive/10 text-destructive hover:bg-destructive hover:text-destructive-foreground transition-colors text-xs font-medium">
                        <X className="h-3 w-3" /> Reject
                      </button>
                    </div>
                  ) : (
          ...
          <div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden">
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                {selected.status === "received" || selected.status === "pending-review" ? (
                  <>
                    <button className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg bg-success px-4 py-2.5 text-sm font-medium text-success-foreground hover:bg-success/90 transition-colors">
                      <Check className="h-4 w-4" /> Approve
                    </button>
                    <button className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg bg-destructive px-4 py-2.5 text-sm font-medium text-destructive-foreground hover:bg-destructive/90 transition-colors">
                      <X className="h-4 w-4" /> Reject
                    </button>
                    <button className="inline-flex items-center justify-center gap-2 rounded-lg border border-border bg-card px-4 py-2.5 text-sm font-medium text-card-foreground hover:bg-secondary transition-colors">
                      <Edit className="h-4 w-4" /> Edit
                    </button>
                  </>
                ) : (
                  <button className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg border border-border bg-card px-4 py-2.5 text-sm font-medium text-card-foreground hover:bg-secondary transition-colors">
        ...
        <div className="space-y-4">
          {/* Upload zone for claims */}
          <div className="rounded-xl border-2 border-dashed border-border bg-card p-8 text-center hover:border-primary/40 transition-colors cursor-pointer">
            <Upload className="h-8 w-8 mx-auto text-muted-foreground mb-3" />
            <p className="text-sm font-medium text-card-foreground">Drag & drop claim documents here, or click to browse</p>
            <p className="text-xs text-muted-foreground mt-1">PDF, PNG, JPG up to 10MB</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative flex-1 min-w-[240px] max-w-md">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
              <input type="text" placeholder="Search claims..." className="w-full rounded-lg border border-input bg-card pl-9 pr-4 py-2 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
              ...
              <tbody>
                <tr className="border-b border-border hover:bg-secondary/40 transition-colors">
                  <td colSpan={7} className="p-8 text-center text-muted-foreground">
                    No claims submitted yet. Click "+ New Claim" to get started.
                  </td>
                </tr>
              </tbody>
            </table>
